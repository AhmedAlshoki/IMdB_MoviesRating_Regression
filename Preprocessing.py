...
movies_dataset = pd.read_csv('Movies_training.csv')
...
def replace_drop():
    ...
    # drop Rows without IMDb
    movies_dataset = movies_dataset[movies_dataset['IMDb'].notnull()]
...
def feature_encoding(dataset_copy):
    ...
    dataset_copy = pd.concat([dataset_copy.drop(min_unique_feature, 1), dataset_copy[min_unique_feature].str.get_dummies(sep=",")], 1)
...
def correlate_data(self):
    ...
    top_feature_columns = movies_corr.index[abs(movies_corr['IMDb'] > 0.05)]
...
def Preprocess():
    ...
    Y = dataset_copy['IMDb']        # target after changing rows
    X = dataset_copy.drop(['IMDb'], axis=1, inplace=False)  # extract features only
    dataset_copy = pd.concat([X, dataset_copy['IMDb']], 1)      # moving IMDb to be the last column
